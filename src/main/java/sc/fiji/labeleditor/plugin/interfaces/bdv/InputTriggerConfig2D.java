...
public class InputTriggerConfig2D {
	...
	public InputTriggerConfig load(JPanel parent) {
		try {
			...
			URL yamlURL = ClassLoader.getSystemResource("bdv2d.yaml");
			if ( yamlURL == null ) {
				System.out.println( "Try to fetch yaml from " + getClass().getClassLoader().getResource("bdv2d.yaml") );
				yamlURL = getClass().getClassLoader().getResource("bdv2d.yaml");
			}
			if ( yamlURL != null ) {
